...
def run_benchmarks(
    ...
    # DRY RUN MODE
    if dry_run:
        app.print("[yellow]DRY RUN MODE[/]")
        app.print("No benchmarks will be executed")
        app.print("No files will be saved")

        benchmark_items = list(config.benchmarks.items())

        if benchmark is not None:
            selected = config.benchmarks.get(benchmark)
        ...
        for benchmark_name, benchmark_data in benchmark_items:
            app.print(f"\n[bold]Benchmark:[/] {benchmark_name}")

            for application_name in benchmark_data.application_names(
                benchmark_name
            ):
                app.print(
                    f"  -> Would run application: {application_name}"
                )
